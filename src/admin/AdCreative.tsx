...
const AdCreative = forwardRef<HTMLDivElement, { format: AdFormat; theme: AdTheme; data: AdData }>(
  ({ format, theme, data }, ref) => {
    ...
    return (
      <div ref={ref} className={`ad ad-${format} ${theme === "light" ? "light" : ""}`}>
        ...
        <div className="cta">{WA} WhatsApp · {data.whatsapp}</div>
        ...
        <div className="web">{GLOBE} {data.website}</div>
      </div>
    );
  }
);
